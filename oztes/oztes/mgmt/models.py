...
class OU(models.Model):
    id = models.UUIDField(
        ...
        default = uuid.uuid4,
        editable = False)
    parent = models.ForeignKey('self', related_name = 'subortinate', on_delete = models.DO_NOTHING,
...
class Emp(models.Model):
    ...
    def aplied_policies(self):
        ...
        for pol in Policy_appied_to_Emp.objects.filter(Emp = self).filter(policy__status = 'en'):
            result.append(pol)
        for pol in Policy_applied_to_OU.objects.filter(OU__in = self.OU.get_list_of_parents()).filter(
                policy__status = 'en'):
            result.append(pol)
        return result
...
class Agent(models.Model):
    id = models.UUIDField(
        ...
        default = uuid.uuid4,
        editable = False)
    Emp = models.ForeignKey('Emp', on_delete = models.CASCADE, null = True, blank = True, help_text = 'Agents owner',
                            related_name = 'owned')
